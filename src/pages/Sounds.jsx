...
export default function SoundsPage() {
  ...
  const seoData = useMemo(() => {
    ...
    const titles = {
      english: `Sounds | Brainrot Memes Funny Meme Sounds & Audio Effects | Download Free ${
        searchTerm ? `${searchTerm} ` : ''
      }Sound Effects`,
      spanish: `Sonidos Graciosos de Memes y Efectos de Audio | Descarga ${
        searchTerm ? `${searchTerm} ` : ''
      }Gratis`,
      ...
      russian: `Смешные Звуки Мемов и Аудио Эффекты | Скачать ${
        searchTerm ? `${searchTerm} ` : ''
      }Бесплатно`,
      arabic: `أصوات ميمز مضحكة وتأثيرات صوتية | تنزيل ${
        searchTerm ? `${searchTerm} ` : ''
      }مجاناً`,
      japanese: `おもしろミーム音とオーディオ効果 | 無料${
        searchTerm ? `${searchTerm} ` : ''
      }サウンドをダウンロード`,
      korean: `재미있는 밈 소리 및 오디오 효과 | 무료 ${
        searchTerm ? `${searchTerm} ` : ''
      }사운드 다운로드`,
      vietnamese: `Âm Thanh Meme Hài Hước & Hiệu Ứng Âm Thanh | Tải ${
        searchTerm ? `${searchTerm} ` : ''
      }Miễn Phí`,
      chinese: `搞笑模因声音和音频效果 | 免费下载${
        searchTerm ? `${searchTerm} ` : ''
      }音效`,
      french: `Sons de Mèmes Drôles et Effets Audio | Téléchargez ${
        searchTerm ? `${searchTerm} ` : ''
      }Gratuitement`,
      italian: `Suoni Divertenti di Meme ed Effetti Audio | Scarica ${
        searchTerm ? `${searchTerm} ` : ''
      }Gratis`,
      turkish: `Komik Meme Sesleri ve Ses Efektleri | Ücretsiz ${
        searchTerm ? `${searchTerm} ` : ''
      }İndirin`,
      hindi: `मजेदार मीम साउंड और ऑडियो इफेक्ट्स | मुफ्त ${
        searchTerm ? `${searchTerm} ` : ''
      }डाउनलोड करें`,
    ...
    const description =
      descriptions[normalizedLanguage] || descriptions.english;
    const title = titles[normalizedLanguage] || titles.english;

    return {
      title,
      description,
      canonical: `https://brainrot-memes.com/${language}/sounds${
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50'>
      <Helmet>
        ...
        <meta
          property='og:image'
          content='https://brainrot-memes.com/og-images/sounds-collection.jpg'
        />
