...
export default function MemeSoundPage() {
  ...
  const handleToggleSave = async () => {
    try {
      ...
      const baseUrl =
        'https://us-central1-meme-soundboard-viral-alarm.cloudfunctions.net';
      const endpoint = isSaved
        ? '/removeSoundFromUserLibrary'
        : '/addSoundToUserLibrary';
                  ...
                  <div className='bg-gray-50 rounded-lg p-4'>
                    <div className='flex items-center justify-between mb-2'>
                      <label className='text-sm font-medium text-gray-700'>
                        Echo Effect (
                        {echoCount === 0 ? 'Off' : `${echoCount * 4} instances`}
                        )
                      </label>
                      <Button
                        size='sm'
                    ...
                    <p className='text-xs text-gray-500 mt-2'>
                      Echo creates up to 20 simultaneous audio instances with
                      varying stereo positioning.
                    </p>
                    {(isPlaying || isConfigLocked) && (
                      <p className='text-xs text-amber-600 mt-2'>
                        Echo configuration cannot be changed while playing or
                        when locked
                      </p>
                    )}
                  </div>
